...
- 115 STRM 直链解析及日志运行 `go test ./emby302/service/emby ./emby302/web/cache ./emby302/util/https ./internal/controllers ./internal/playback ./internal/helpers`。使用本地 HTTP 替身确认自有接口解析不访问 CDN、UA 和签名地址完整传递、异常响应回退、第三方多级解析及播放器 `307` 响应；日志回归覆盖首次取链、刷新、副本来源、连续缓存命中不重复长地址、HEAD 检查和副本身份字段。
