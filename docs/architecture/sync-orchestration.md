...
全局 Cron 每小时的维护入口同时清理请求统计和 115 多端播放残留。多端播放维护逐账号执行，重叠触发会合并；它与播放请求共用同一编排器以避开活跃目录，不创建同步任务记录，也不受多端播放开关关闭影响。目录归属、保留时间和清理预算见 [115 多端播放链路](upload-and-strm-processing.md#115-多端播放链路)。
