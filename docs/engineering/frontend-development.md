...
- 接口速率页的“同时上传任务数”复用现有设置读写接口，输入范围来自 `THREAD_LIMITS`，保存前再次检查整数和范围；清空或非法输入不能直接提交。保存后的并发变化和暂停语义见 [上传队列并发](../operations/configuration.md#上传队列并发)。
...
- 下载、上传队列顶部统一显示“剩余 · 排队 · 处理中”，复用已有 `queue_status`：排队取 `pending`，处理中取 `processing`，剩余为两者之和。统计覆盖整个队列，不随分页或状态筛选变化；完成、失败和取消任务不计入剩余，重试回到等待状态后重新计入。下载处理中包含已预取的任务，上传处理中包含等待及正在执行完成处理的任务，因此不能将该数字等同于正在传输的文件数。统计沿用现有快照和刷新链路，不新增请求。
...
- 长任务优先使用现有事件流；HTTP 状态接口用于首次加载、手动刷新和断线恢复。需要最终收敛的活动队列即使 SSE 已连接，也必须保留低频 HTTP 权威快照兜底；SSE 事件可立即触发刷新，但同一页面的快照请求必须通过 request gate 合并，禁止并发请求。115 OAuth、二维码授权等短生命周期外部流程沿用各自轮询，不并入通用队列事件。
- 所有活动任务轮询都必须遵循页面生命周期：页面隐藏时暂停非必要 timer 和请求，恢复可见时立即进行一次快照收敛；轮询必须防止 in-flight 请求重叠。Emby、更新和备份/恢复轮询要以 generation 或等价失活令牌保护异步回调，页面卸载、停用、取消或任务终态后，飞行请求不得写回状态或重新创建 timer。备份 store 虽无组件卸载钩子，也必须在显式停止和新一轮 generation 后使旧回调失效。
...
- 页面内容中的局部 `v-loading` 遮罩由 `assets/components.css` 统一限制在移动导航背景遮罩和侧边栏之下，同时继续阻止加载区域内的操作；全屏加载与模态对话框保留 Element Plus 的层级。不得通过隔离整个 `.main-content` 的层叠上下文修复局部遮罩，否则就地挂载的对话框会落到侧边栏下方。
...
- 空状态统一使用 `el-empty`，`image-size` 按容器层级取三级：页面级列表空态用 80，卡片内组件（图表、统计卡）用 60，小内嵌信息块用 40；新增页面按所在层级取值，不引入新的尺寸。
